...
$getDebug = array_key_exists("DebugOn", $_POST) ? $_POST["DebugOn"] : "0";
...
// This is the display title for 'Error' & 'Receipt' pages 
$title = $_POST["Title"];
...
$payClientIP = array_key_exists("HostName", $_POST) ? $_POST["HostName"] : "";

// Payment Client Port number. In a production system you would not pass 
// in this value, but hard it into the program
$payClientPort = intval(array_key_exists("Port", $_POST) ? $_POST["Port"] : "");
...
// Step 2 - Test the Payment Client socket was created successfully
// ================================================================
...
// ===================================================
// Add any supplementary data to the Digital Order. This is a local command that
// only communicates to the Payment Client and nothing is sent to the Payment
// Server.
...
if (!$errorExists) {
    if (strlen($merchTxnRef) > 0) { 
        $cmdResponse = sendCommand($payClientSocket, "7,MerchTxnRef,$merchTxnRef", $debug);
        if (substr($cmdResponse,0,1) != $OK) {
            // Display an error message as the command failed
            $message = "(6) Failed to add supplementary data using addDigitalOrderField command - MerchTxnRef. Response=".$cmdResponse;
            $errorExists = true;
        }
    }
}
...
if (!$errorExists) {
    ...
    if (strlen($ticketNo) > 0) {
        $cmdResponse = sendCommand($payClientSocket, "7,TicketNo,$ticketNo", $debug);
        if (substr($cmdResponse,0,1) != $OK) {
            // Display an error message as the command failed
            $message = "(6) Failed to add supplementary data using addDigitalOrderField command - TicketNo. Response=".$cmdResponse;
            $errorExists = true;
        }
    }
}

// Step 4 - Generate Digital Order
// *******************************
...
if (!$errorExists) {
    ...
    // Add to the ReturnURL the HostName & Port to use in the Digital Receipt
    // processing page. In production code you could simply hard code these 
    // values into both the Digital Order and Digital Receipt processing pages.
    // If you need to add other session variables to keep track of a client's
    // session, you can use this method and they will be returned appended to 
    // the Digital receipt.
    $returnURL = $_POST["ReturnURL"]."&HostName=".$payClientIP."&Port=".$payClientPort."&Title=".urlencode($title)."&AgainLink=".urlencode($againLink)."&DebugOn=".$getDebug;

	// (This primary command also receives the encrypted Digital Receipt)
    $cmdResponse = sendCommand($payClientSocket,"2,$orderInfo,$merchantID,$amount,$locale,$returnURL",$debug);
    if (substr($cmdResponse,0,1) != $OK) {
        ...
        // Display an error message as the command failed
        $message = "(11) Digital Order has not created correctly - getDigitalOrder($orderInfo,$merchantID,$locale,$returnURL) failed.";
        $errorExists = true;
    } else {
        // get the encrypted Digital Order from the response
        $digitalOrder = substr($cmdResponse,2);
    }
}


// Step 5 - We are finished with the Payment Client so tidy up
// ===========================================================
...
// FINISH TRANSACTION - Redirect the customer using the Digital Order
// ==================================================================
if (!$errorExists) {
	if (!$debug) {
		...
		echo("<a href='$digitalOrder'>Click Here To Continue</a>");
	}
} else {
...
// the following output will only be displayed if an error has occurred.
?>
